...
export async function GET(_request: Request, { params }: RouteParams) {
  const { id } = params;
  try {
    const result = await query<WorstCourierCarryForwardReport>('SELECT * FROM duc_worst_courier_carry_forward_reports WHERE id = $1', [id]);
    if (result.length === 0) {
      return NextResponse.json<ApiResponse>({ error: 'Report not found', status: 404 });
    }
    return NextResponse.json<ApiResponse<WorstCourierCarryForwardReport>>({ data: result[0], status: 200 });
  } catch (error: any) {
    console.error(`GET /api/worst-courier-carry-forward-reports/${id} Error:`, error);
...
export async function DELETE(_request: Request, { params }: RouteParams) {
  const { id } = params;
  try {
    const result = await query('DELETE FROM duc_worst_courier_carry_forward_reports WHERE id = $1 RETURNING id', [id]);
    if (result.length === 0) {
      return NextResponse.json<ApiResponse>({ error: 'Report not found', status: 404 });
    }
    return NextResponse.json<ApiResponse<{ message: string }>>({ data: { message: `Report ${id} deleted successfully` }, status: 200 });
  } catch (error: any) {
    console.error(`DELETE /api/worst-courier-carry-forward-reports/${id} Error:`, error);
