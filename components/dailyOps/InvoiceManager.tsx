...
const InvoiceManager: React.FC = () => {
  ...
  const executeGenerateInvoice = async () => {
    ...
    const newLines: InvoiceLine[] = relevantSchedules.map((ws) => {
      ...
      return {
        id: 0, // dummy value, backend will assign correct id
        invoice_id: "", // empty string for draft, backend will assign correct invoice_id
        date: ws.date,
        ...
        <ul className="space-y-2">
          {filteredInvoices.map((inv) => (
            <li key={inv.id} className="p-3 bg-white shadow rounded flex justify-between">
              <span>{inv.invoice_number || inv.id.toString().slice(-6)}</span>
